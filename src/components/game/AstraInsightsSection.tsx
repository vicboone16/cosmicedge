...
export function AstraInsightsSection({
  ...
  const structured: AstraResponse | null = data?.structured || null;
  ...
  if (isLoading) {
    return (
      <section className="flex items-center justify-center py-8 gap-2">
        ...
        <span className="text-xs text-muted-foreground">Astra is reading the stars...</span>
      </section>
    );
  }
  ...
  return (
    <section>
      ...
      <AstraStructuredResponse data={structured} compact />
    </section>
  );
}
