...
import { ArrowLeft, ChevronLeft, ChevronRight, Moon, Star, Orbit } from "lucide-react";
    ...
    <div className="min-h-screen">
      <header className="px-4 pt-12 pb-4 bg-background/80 backdrop-blur-xl border-b border-border/50">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold font-display">Cosmic Calendar</h1>
        </div>
        <p className="text-xs text-muted-foreground mt-1">Week of {format(weekStart, "MMMM d, yyyy")}</p>
      </header>
      ...
      <div className="flex items-center justify-between px-4 py-3">
        ...
        <button onClick={() => setWeekStart(startOfWeek(new Date(), { weekStartsOn: 1 }))} className="text-xs text-primary font-medium hover:underline">
          This Week
        </button>
        <button onClick={() => setWeekStart(addDays(weekStart, 7))} className="p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors">
