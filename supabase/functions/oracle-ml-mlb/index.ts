...
/**
 * oracle-ml-mlb — MLB pregame score + ML predictions
 * Method: Poisson runs model (SP + offense + defense)
 */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const season = new Date().getFullYear(); // MLB season = calendar year

    let gamesQ = sb.from("games").select("*").eq("league", "MLB").in("status", ["scheduled", "live"]);
    if (singleGameId) {
    ...
    if (gErr) throw gErr;
    if (!games?.length) {
      return new Response(JSON.stringify({ sport: "mlb", model_name: "oracle_ml", model_version: "v1", games_processed: 0, inserted: 0 }), {
    ...
    const { data: paceRows } = await sb.from("team_season_pace").select("*")
      .eq("season", season).eq("league", "MLB").in("team_abbr", allTeams);
    ...
    for (const game of games) {
      ...
      const hAvgRuns = Number(hP?.avg_points) || LEAGUE_AVG_RUNS;
      const aAvgRuns = Number(aP?.avg_points) || LEAGUE_AVG_RUNS;
      const hAvgAllowed = Number(hP?.avg_points_allowed) || LEAGUE_AVG_RUNS;
      const aAvgAllowed = Number(aP?.avg_points_allowed) || LEAGUE_AVG_RUNS;
      const hNet = Number(hP?.net_rating) || 0;
      const aNet = Number(aP?.net_rating) || 0;
      const hGP = hP?.games_played || 0;
      const aGP = aP?.games_played || 0;

      const hOffStr = hAvgRuns / LEAGUE_AVG_RUNS;
      const aOffStr = aAvgRuns / LEAGUE_AVG_RUNS;
      const hDefStr = hAvgAllowed / LEAGUE_AVG_RUNS;
      const aDefStr = aAvgAllowed / LEAGUE_AVG_RUNS;

      const muHome = LEAGUE_AVG_RUNS * hOffStr * (2 - aDefStr) + HOME_ADV;
      const muAway = LEAGUE_AVG_RUNS * aOffStr * (2 - hDefStr);
      ...
      const blowout = clamp(Math.abs(muDiff) / 5, 0, 1);
      ...
      rows.push({
        game_id: game.id, sport: "MLB", model_name: "oracle_ml", model_version: "v1",
        run_ts: runTs,
        ...
        p_home_win_ci_low: +clamp(pHomeWin - 0.15, 0.01, 0.99).toFixed(4),
        p_home_win_ci_high: +clamp(pHomeWin + 0.15, 0.01, 0.99).toFixed(4),
        qtr_wp_home: null, qtr_fair_ml: null,
        features_json: {
          home_avg_runs: hAvgRuns, home_avg_allowed: hAvgAllowed, home_gp: hGP,
          away_avg_runs: aAvgRuns, away_avg_allowed: aAvgAllowed, away_gp: aGP,
    ...
    return new Response(JSON.stringify({
      sport: "mlb", model_name: "oracle_ml", model_version: "v1",
      games_processed: games.length, inserted: rows.length, run_ts: runTs,
    }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });

  } catch (err) {
    console.error("oracle-ml-mlb error:", err);
