// fetch-team-props — Team-level props from SGO (team totals, team spreads, etc.)
// Stores into sgo_market_odds with is_player_prop=false
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: dbGames } = await supabase
      .from("games").select("id, home_abbr, away_abbr, start_time, external_id")
      .eq("league", league)
    ...
    if (!dbGames?.length) {
      return new Response(
        JSON.stringify({ success: true, events: 0, team_props: 0, reason: "no_games" }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    }

    // Fetch SGO events with odds
    ...
    if (json.notice) console.warn(`[TeamProps] Notice: ${json.notice}`);
    console.log(`[TeamProps] ${league}: ${events.length} SGO events`);

    let totalTeamProps = 0;
    ...
    for (const event of events) {
      ...
      for (const [oddID, oddData] of Object.entries(odds) as [string, any][]) {
        ...
        // Team props: statEntityID is "home" or "away" (not "all" and not a player)
        const isTeamProp = statEntityID === "home" || statEntityID === "away";
        if (!isTeamProp) continue;
        ...
        const period = periodID && periodID !== "game" ? periodID : "full";
        ...
        if (consensusOdds != null || consensusLine != null) {
          rows.push({
            ...
            bet_type: betType, side, period, stat_entity_id: statEntityID,
            stat_id: statID, player_name: null, is_player_prop: false,
      ...
      if (rows.length > 0) {
        console.log(`[TeamProps] ${matched.home_abbr} vs ${matched.away_abbr}: ${rows.length} team props`);
        for (let i = 0; i < rows.length; i += BATCH) {
          ...
          if (error) console.error("Team props upsert error:", error.message);
        }
        totalTeamProps += rows.length;
      }
    }

    return new Response(
      JSON.stringify({ success: true, league, events: events.length, team_props_stored: totalTeamProps, fetched_at: new Date().toISOString() }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } },
    );
  } catch (error: unknown) {
    const msg = error instanceof Error ? error.message : String(error);
    console.error("fetch-team-props error:", msg);
    return new Response(
