// fetch-alt-lines — Alternate spreads, totals, and moneylines from SGO
// Filters for non-main-line odds and stores in sgo_market_odds
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!dbGames?.length) {
      return new Response(
        JSON.stringify({ success: true, events: 0, alt_lines: 0, reason: "no_games" }),
    ...
    if (json.notice) console.warn(`[AltLines] Notice: ${json.notice}`);
    console.log(`[AltLines] ${league}: ${events.length} SGO events`);
    ...
    for (const event of events) {
      ...
      for (const [oddID, oddData] of Object.entries(odds) as [string, any][]) {
        // Only alternate lines (non-main-line)
        ...
        if (consensusOdds != null || consensusLine != null) {
          rows.push({
            ...
            is_alternate: true, bookmaker: "consensus",
          ...
          for (const [bkId, bkData] of Object.entries(oddData.byBookmaker) as [string, any][]) {
            ...
            if (bkOdds != null || bkLine != null) {
              rows.push({
                ...
                bet_type: betType, side, period, stat_entity_id: statEntityID,
                stat_id: statID, player_name: playerName, is_player_prop: isPlayerProp,
                is_alternate: true, bookmaker: bkId,
      ...
      if (rows.length > 0) {
        console.log(`[AltLines] ${matched.home_abbr} vs ${matched.away_abbr}: ${rows.length} alt lines`);
        for (let i = 0; i < rows.length; i += BATCH) {
          ...
          if (error) console.error("Alt lines upsert error:", error.message);
        }
        totalAltLines += rows.length;
      }
    }

    return new Response(
      JSON.stringify({ success: true, league, events: events.length, alt_lines_stored: totalAltLines, fetched_at: new Date().toISOString() }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } },
    );
  } catch (error: unknown) {
    const msg = error instanceof Error ? error.message : String(error);
    console.error("fetch-alt-lines error:", msg);
    return new Response(
