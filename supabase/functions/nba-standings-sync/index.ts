...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      console.error("BDL standings error:", resp.status, text);
      
      // Fallback: compute standings from games table
      return await computeStandingsFromGames(supabase, corsHeaders);
    }
    ...
    if (standings.length === 0) {
      // Fallback to computed standings
      return await computeStandingsFromGames(supabase, corsHeaders);
    }

    // Normalize team abbreviations
    const ABBR_NORMALIZE: Record<string, string> = {
      PHO: "PHX", GS: "GSW", SA: "SAS", NY: "NYK", NO: "NOP", CHA: "CHA",
      BKN: "BKN", VEG: "VGK",
    };
    ...
    const rows = standings.map((s: any) => {
      ...
      return {
        team_abbr: abbr,
        team_name: s.team?.full_name || s.team?.name || abbr,
        ...
        seed: s.seed ?? s.conference_rank ?? null,
        streak: s.streak || null,
        last_10: s.last_ten || null,
        pct: s.wins > 0 ? (s.wins / (s.wins + s.losses)).toFixed(3) : "0.000",
        season: 2025,
        league: "NBA",
    ...
    // Upsert into nba_standings
    const { error } = await supabase
      .from("nba_standings")
      .upsert(rows, { onConflict: "team_abbr,season" });
...
async function computeStandingsFromGames(supabase: any, corsHeaders: Record<string, string>) {
  const now = new Date();
  ...
  if (error || !games?.length) {
    return new Response(JSON.stringify({ success: false, error: "No games found", computed: true }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
  ...
  const rows = Object.entries(teams).map(([abbr, t]) => ({
    ...
    road_wins: t.roadW,
    road_losses: t.roadL,
    pct: t.wins > 0 ? (t.wins / (t.wins + t.losses)).toFixed(3) : "0.000",
    season: 2025,
    league: "NBA",
    updated_at: new Date().toISOString(),
  }));
  ...
  return new Response(JSON.stringify({ success: !upsertErr, count: rows.length, computed: true }), {
    headers: { ...corsHeaders, "Content-Type": "application/json" },
  });
}
