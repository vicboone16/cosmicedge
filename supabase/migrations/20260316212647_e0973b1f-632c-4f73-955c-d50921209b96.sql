...
-- Drop existing PBP pipeline views in dependency order
...
-- 1) Parsed events from nba_pbp_events (BDL source with raw jsonb + participants)
...
SELECT
    ...
    CASE
        ...
        WHEN e.description ILIKE '%assist%' THEN 'ASSIST'
        WHEN e.description ILIKE '%steal%' THEN 'STEAL'
        WHEN e.description ILIKE '%block%' THEN 'BLOCK'
        ...
        WHEN e.description ILIKE '%enters the game%' OR e.description ILIKE '%substitution%' THEN 'SUBSTITUTION'
        ...
        WHEN e.description ILIKE '%makes%'
             AND e.description NOT ILIKE '%3-pt%'
             AND e.description NOT ILIKE '%free throw%'
        THEN 'SHOT_MADE_2'
        WHEN e.description ILIKE '%misses%'
             AND e.description NOT ILIKE '%3-pt%'
             AND e.description NOT ILIKE '%free throw%'
        THEN 'SHOT_MISSED_2'
        ELSE 'OTHER'
    END AS parsed_event_type,
    -- Resolve BDL player ID to internal UUID via players.external_id
    pl.id AS resolved_player_id
FROM public.nba_pbp_events e
LEFT JOIN public.players pl ON pl.external_id = e.player_id;

-- 2) Participants expansion from raw->participants array
...
SELECT
    ...
    ROW_NUMBER() OVER (
        PARTITION BY pe.game_id, pe.event_id
        ORDER BY part.ordinality
    ) AS participant_order
FROM pbp_parsed_events pe
CROSS JOIN LATERAL jsonb_array_elements(
    COALESCE(pe.play_json->'participants', '[]'::jsonb)
) WITH ORDINALITY AS part(value, ordinality)
LEFT JOIN public.players pl ON pl.external_id = part.value::text;

-- 3) Stat deltas - participant-aware
...
-- Participant-aware assists: non-primary participant on made shots with "assist" in description
...
WHERE ep.event_type IN ('SHOT_MADE_2','SHOT_MADE_3')
  AND ep.participant_player_id IS NOT NULL
  AND ep.participant_player_id IS DISTINCT FROM ep.primary_player_id
  AND ep.description ILIKE '%assist%'
-- Participant-aware steals: non-primary on turnovers with "steal"
...
FROM pbp_event_participants ep
WHERE ep.event_type = 'TURNOVER'
  AND ep.participant_player_id IS NOT NULL
  AND ep.participant_player_id IS DISTINCT FROM ep.primary_player_id
  AND ep.description ILIKE '%steal%'
-- Participant-aware blocks: non-primary on missed shots with "block"
...
FROM pbp_event_participants ep
WHERE ep.event_type IN ('SHOT_MISSED_2','SHOT_MISSED_3')
  AND ep.participant_player_id IS NOT NULL
  AND ep.participant_player_id IS DISTINCT FROM ep.primary_player_id
  AND ep.description ILIKE '%block%';
...
-- 7) PBP patch view with PIE numerator
...
-- 8) Substitution events for future lineup/plus-minus tracking
...
SELECT
    ...
    ep.team_id,
    ep.description,
    ep.participant_bdl_id,
    ep.participant_player_id,
    ep.participant_order,
    CASE
        WHEN ep.participant_order = 1 THEN 'PLAYER_IN'
        WHEN ep.participant_order = 2 THEN 'PLAYER_OUT'
        ELSE 'UNKNOWN'
