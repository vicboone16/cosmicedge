...
-- 1. Sportsbook scoring rules table
CREATE TABLE IF NOT EXISTS fantasy_scoring_rules (
    sportsbook TEXT PRIMARY KEY,
    points_weight FLOAT NOT NULL DEFAULT 1,
    rebounds_weight FLOAT NOT NULL DEFAULT 1.2,
    assists_weight FLOAT NOT NULL DEFAULT 1.5,
    steals_weight FLOAT NOT NULL DEFAULT 3,
    blocks_weight FLOAT NOT NULL DEFAULT 3,
    turnovers_weight FLOAT NOT NULL DEFAULT -1,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

INSERT INTO fantasy_scoring_rules (sportsbook, points_weight, rebounds_weight, assists_weight, steals_weight, blocks_weight, turnovers_weight) VALUES
('PrizePicks', 1, 1.2, 1.5, 3, 3, -1),
('Underdog', 1, 1.2, 1.5, 3, 3, -1),
('FanDuel', 1, 1.2, 1.5, 3, 3, -1),
('DraftKings', 1, 1.2, 1.5, 3, 3, -1)
ON CONFLICT (sportsbook) DO NOTHING;

-- 2. Drop and recreate views in dependency order
DROP VIEW IF EXISTS fantasy_scores;
DROP VIEW IF EXISTS player_stats_by_window;

-- 3. player_stats_by_window with quarter/half breakdowns from existing player_game_stats
CREATE VIEW player_stats_by_window AS
SELECT
  pgs.game_id,
  pgs.player_id,
  p.name AS player_name,
  pgs.team_abbr,
  -- Full game (period = 'full')
...
-- 4. Fantasy scores joined to scoring rules for sportsbook-aware calculation
CREATE VIEW fantasy_scores AS
SELECT
  s.game_id,
  s.player_id,
  s.player_name,
