-- Sync missing np_ functions and views from Live to Test.
-- These objects exist in Live but were absent from Test, causing the Lovable
-- publish pipeline to generate DROP statements that fail due to dependencies.
-- Adding them here makes the Test/Live schema diff empty and unblocks deploys.

-- 1. np_norm_cdf: standard normal CDF used by edgescore computations
...
-- 2. np_apply_edgescore_v11: batch-update edge_score on nebula_prop_predictions
...
-- 3. np_persist_edgescore_v11: persist edge_score_v11 column
...
-- 4. np_player_prop_stat_long: unpivots player_game_stats into prop_type/stat_value rows
...
-- 5. np_v_closing_lines: last odds snapshot per prop/book/side
...
-- 6. np_v_backtest_overlay: predictions joined with closing lines
...
LEFT JOIN np_v_closing_lines cl
    ON cl.game_id = p.game_id
   AND cl.player_id = p.player_id
   AND cl.prop_type = p.prop_type
   AND cl.book = p.book
   AND cl.side = p.side;

-- 7. np_v_backtest_results: overlay joined with actual stats and win flag
...
LEFT JOIN np_player_prop_stat_long s
    ON s.game_id = o.game_id
   AND s.player_id = o.player_id
   AND s.prop_type = o.prop_type;
