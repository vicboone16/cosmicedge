...
BEGIN
  ...
  team_possessions AS (
    SELECT
      t.team_abbr,
      t.game_id,
      -- Use actual game scores to estimate possessions
      -- Both teams play the same number of possessions per game
      CASE WHEN t.team_abbr = t.home_abbr
        THEN GREATEST(COALESCE(t.home_score,0) + COALESCE(t.away_score,0), 160) / 2.0 / 1.08
        ELSE GREATEST(COALESCE(t.home_score,0) + COALESCE(t.away_score,0), 160) / 2.0 / 1.08
      END AS poss,
      CASE WHEN t.team_abbr = t.home_abbr THEN t.home_score ELSE t.away_score END AS pts,
      CASE WHEN t.team_abbr = t.home_abbr THEN t.away_score ELSE t.home_score END AS pts_allowed
