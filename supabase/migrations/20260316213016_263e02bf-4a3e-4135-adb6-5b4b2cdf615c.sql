...
-- 6) PBP patch with PIE
CREATE VIEW live_player_tracking_pbp_patch AS
SELECT s.game_id, s.team_id, s.player_id, COALESCE(s.player_name, pl.name) AS player_name,
s.game_points AS points, s.game_rebounds AS rebounds, s.game_assists AS assists, s.game_steals AS steals, s.game_blocks AS blocks, s.game_turnovers AS turnovers,
s.game_two_pa AS two_pa, s.game_two_pm AS two_pm, s.game_three_pa AS three_pa, s.game_three_pm AS three_pm, s.game_fta AS fta, s.game_ftm AS ftm, s.game_oreb AS oreb, s.game_dreb AS dreb,
